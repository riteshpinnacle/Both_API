...
//                val response2 = api2.searchBarcode(BarcodeRequest(searchValue = barcode))
                val response2 = if (key == null) {
                    api2.searchBarcode(BarcodeRequest(searchValue = barcode))
                } else {
                ...
                val api3 = createAPI3()
                val response3 = api3.searchBarcode(BarcodeAdd(barcode))
                val result3 = response3.message
                binding.txtMessage2.text = "$result3 for $barcode"

                val api4 = createAPI4()
                val response4 = api4.searchBarcode(BarcodeCountInput(barcode))
                val result4 = response4.count
                binding.txtMessage3.text = "$barcode has been scanned successfully $result4 times"
